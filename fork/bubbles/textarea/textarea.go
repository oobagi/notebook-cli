...
// CursorSetChar sets the character under the virtual cursor.
func (m *Model) CursorSetChar(char string) {
	m.virtualCursor.SetChar(char)
}

// CursorView returns the rendered view of the virtual cursor.
...
// VisualLineCount returns the total number of visual (wrapped) lines across
// all raw lines, using the textarea's own wrapping logic and current width.
...
// Wrap splits a slice of runes into lines that fit within the given width,
// using word wrapping with hard-break fallback for words exceeding the width.
...
func wrap(runes []rune, width int) [][]rune {
	...
	// Word wrap the runes, with hard-breaking for words that exceed width.
	for _, r := range runes {
		...
		if spaces > 0 { //nolint:nestif
			...
			// Hard-break: if the line is still wider than width after
			// appending, split it at character boundaries.
			row, lines = hardBreakLine(row, lines, width)
		} else {
...
// hardBreakLine splits lines[row] at character boundaries if it exceeds width.
// Returns the updated row index and lines slice.
func hardBreakLine(row int, lines [][]rune, width int) (int, [][]rune) {
	for uniseg.StringWidth(string(lines[row])) > width {
		// Find the rune index where we exceed width.
		w := 0
		breakAt := len(lines[row])
		...
		if breakAt == 0 {
			// Single rune wider than width (e.g. CJK in 1-col terminal);
			// put at least one rune per line to avoid infinite loop.
			breakAt = 1
		}
		overflow := make([]rune, len(lines[row][breakAt:]))
		copy(overflow, lines[row][breakAt:])
		lines[row] = lines[row][:breakAt]
		row++
